...
{
    public partial class F_Cargos : Frm_Lista_Base
    {
        SqlConnection _Conexion = new SqlConnection(ConfigurationManager.ConnectionStrings["MiConexion"].ToString());

        public F_Cargos(int idusuario, int idRol, string usuario)
        {
            InitializeComponent();
            lb_usuario.Text = usuario;
            lbiduser.Text = idusuario.ToString();
            lbidrol.Text = idRol.ToString();
          //  MessageBox.Show("Usuario " + usuario.ToString() + " id user " + idusuario.ToString() 
               // + " Id rol "+ idRol.ToString() + " ", "Validacion de Datos", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
      ...
      private void F_Cargos_Evento_Actualizar_1(object sender, EventArgs e)
        {
            try
            {   
                ...
                if (resultado > 0) /*Si tiene persmisos haga esto*/
                {
                    ...
                    mCargos frm = new mCargos();
                    frm.Modo = "M";
                    frm.Id_Cargo = Convert.ToInt32(this.lstDatos.SelectedItems[0].Text);//.ToString
                ...
                {
                    MessageBox.Show("El usuario no tiene permisos para realizar esta acción", "Validación de Datos", MessageBoxButtons.OK, MessageBoxIcon.Error); ; return;
                }
        ...
        private void F_Cargos_Evento_Consultar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
        ...
        private void F_Cargos_Evento_Eliminar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
                ...
                if (resultado > 0) /*Si tiene persmisos haga esto*/
                {
                    ...
                    mCargos frm = new mCargos();
                    frm.Modo = "E";
                    frm.Id_Cargo = Convert.ToInt32(this.lstDatos.SelectedItems[0].Text);
                    frm.MostrarIngresar = false;
                    frm.MostrarConsultar = false;
        ...
        private void F_Cargos_Evento_Ingresar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
        ...
        private void F_Cargos_Evento_Salir(object sender, EventArgs e)
        {
            this.Close();
        }
    }
