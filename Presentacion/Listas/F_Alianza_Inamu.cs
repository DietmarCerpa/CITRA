...
{
    public partial class F_Alianza_Inamu : Frm_Lista_Base
    {
        SqlConnection _Conexion = new SqlConnection(ConfigurationManager.ConnectionStrings["MiConexion"].ToString());
        public F_Alianza_Inamu(int idusuario, int idRol, string usuario)
        {
            InitializeComponent();
        ...
        Inamus IINAMUS;
        private void F_Alianza_Inamu_Load(object sender, EventArgs e)
        {
            IINAMUS = new Inamus();
        ...
        private void F_Alianza_Inamu_Evento_Actualizar_1(object sender, EventArgs e)
        {
            try
            {                          
                ...
                if (resultado > 0) /*Si tiene persmisos haga esto*/
                {
                    ...
                    mAlianza_Inamu frm = new mAlianza_Inamu();
                    frm.Modo = "M";
                    frm.Id_Contacto = Convert.ToInt32(this.lstDatos.SelectedItems[0].Text);//.ToString
                    frm.MostrarIngresar = false;
                    frm.MostrarEliminar = false;
                    frm.MostrarConsultar = false;
                    frm.Controls["Txt_Id_Contacto"].Enabled = false;
                    frm.Controls["Txt_Nombre_Contacto"].Enabled = false;
                    frm.ShowDialog();
                    F_Alianza_Inamu_Load(null, null);
                }
        ...
        private void F_Alianza_Inamu_Evento_Consultar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
                ...
                if (resultado > 0) /*Si tiene persmisos haga esto*/
                {
                    BAlianza_Inamu frm = new BAlianza_Inamu();
                    frm.ShowDialog();
                }
                else { MessageBox.Show("El usuario no tiene permisos para realizar esta acción", "Validación de Datos", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        private void F_Alianza_Inamu_Evento_Eliminar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
                ...
                if (resultado > 0) /*Si tiene persmisos haga esto*/
                {
                    ...
                    mAlianza_Inamu frm = new mAlianza_Inamu();
                    frm.Modo = "E";
                    frm.Id_Contacto = Convert.ToInt32(this.lstDatos.SelectedItems[0].Text);
                    frm.MostrarIngresar = false;
                    frm.MostrarConsultar = false;
                    ...
                    frm.Controls["Txt_Correo_Inamu"].Enabled = false;
        ...
        private void F_Alianza_Inamu_Evento_Ingresar(object sender, EventArgs e)
        {
            try
            {
                string CadenaSql = "SELECT  [CITRA].[dbo].Usuarios.Id_Usuario from [CITRA].[dbo].Permisos_x_Rol " +
        ...
        private void F_Alianza_Inamu_Evento_Salir(object sender, EventArgs e)
        {
            this.Close();
        }
    }
